...
/**
 * ParallelIteratesTest
 * @author debop sunghyouk.bae@gmail.com
 */
class ParallelIteratesTest : AbstractCoreKotlinTest() {
  ...
  @Test fun `parallel count`() {
    assertThat(xs.parCount { it % 2 == 0 }).isEqualTo(500)
  }
  ...
  @Test fun `parallel forEach`() {
    ...
    assertThat(even.asList().take(3)).isEqualTo(listOf(0, 2, 4))
  }
  ...
  @Test fun `parallel map`() {
    ...
    assertThat(times).hasSize(1000)
    assertThat(times.take(5)).isEqualTo(listOf(0, 2, 4, 6, 8))
  }

  @Test fun `parallel flatMap`() {
    val fm = xss.parFlatMap { x: List<Int> -> x.map { it * 2 } }

    assertThat(fm.size).isEqualTo(9)
    log.debug("flatMap=$fm")
    assertThat(fm.last()).isEqualTo(16)
  }
  ...
  @Test fun `parallel filter map`() {
    ...
    assertThat(fm.size).isEqualTo(500)
    assertThat(fm.take(5)).isEqualTo(listOf(0, 4, 8, 12, 16))
    assertThat(fm.last()).isEqualTo(1996)
  }

  @Test fun `parallel group by`() {
    val groupBy: MutableMultimap<Int, Int> = xs.parGroupBy { it % 2 }

    assertThat(groupBy.keysView().size()).isEqualTo(2)
    assertThat(groupBy[0].size).isEqualTo(500)
    assertThat(groupBy[1].size).isEqualTo(500)
  }

  @Test fun `parallel aggregate by`() {
    val agg: MutableMap<Int, Int> = xs.parAggregateBy({ 1 }, { 0 }, { acc, item -> acc + item + 1 })

    assertThat(agg.keys.size).isEqualTo(1)
    log.debug("agg=${agg[1]}")
    assertThat(agg[1]).isEqualTo(500500)

    val agg2 = xs.parAggregateBy({ it % 10 }, { 0 }, { acc, item -> acc + item + 1 })
    assertThat(agg2.keys.size).isEqualTo(10)
    log.debug("agg2=${agg2.values}")

    (0 until 10).forEach { i ->
      assertThat(agg2[i]).isEqualTo(49600 + 100 * i)
    }
  }
}