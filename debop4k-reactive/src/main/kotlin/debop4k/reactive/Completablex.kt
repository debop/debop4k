...
fun <T> completableOf(f: KCallable<T>): Completable
    = Completable.fromCallable { f.call() }

fun <T> completableOf(f: Callable<T>): Completable
    = Completable.fromCallable(f)

fun Action0.toCompletable(): Completable = Completable.fromAction(this)

fun <T> Function0<T>.toCompletable(): Completable
    = Completable.fromCallable { this.invoke() }

fun <T> KCallable<T>.toCompletable(): Completable
    = Completable.fromCallable { this.call() }

fun <T> Callable<T>.toCompletable(): Completable
    = Completable.fromCallable { this.call() }
