 ...
 */
class Cons<T>(override val head: T, val tailFunc: () -> Permutation<T>) : Permutation<T>() {

  @Volatile
  private var tailOrNull: Permutation<T>? = null

  private val lock = ReentrantLock()

  override val tail: Permutation<T> by lazy {
    if (!isTailDefined) {
      synchronized(lock) {
        if (!isTailDefined) {
          tailOrNull = tailFunc()
